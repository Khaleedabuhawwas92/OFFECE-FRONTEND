// src/utils/printStatementReport.js
export async function printStatementReport({
  customerName = "",
  customerAddress = "",
  periodFrom = "",
  periodTo = "",
  rows = [], // [{ invoice_number, company, created_at, value }]
}) {
  const fmtMoney = (n) => {
    const x = Number(n || 0);
  ...
  const rowsHtml = (rows || [])
    .map((r) => {
      return `
        <tr>
          <td class="center" dir="ltr">${esc(r.invoice_number)}</td>
          <td>${esc(r.company)}</td>
          <td class="center" dir="ltr">${esc(r.created_at)}</td>
          <td class="right" dir="ltr">${fmtMoney(r.value)}</td>
        </tr>
      `.trim();
    })
    .join("\n");

  const totalValue = fmtMoney(
    (rows || []).reduce((acc, r) => acc + (Number(r.value || 0) || 0), 0),
  );

  const PAGE_NO = "1";
  const PAGE_TOTAL = "1";

  // ✅ القالب اللي عطيتني إياه (نفسه) مع placeholders
...
<html lang="ar" dir="rtl">
    ...
    <div class="page">
      <div class="top">
        ...
        <div class="titleBlock">
          ...
          <div class="pageNo">الصفحة: ${PAGE_NO} من ${PAGE_TOTAL}</div>
        </div>
      </div>
      ...
      <div class="infoRow">
        <div class="customer">
          ...
          <div class="lines">${esc(customerAddress)}</div>
        </div>

        <table class="periodBox">
          <tr><th>فترة الكشف</th></tr>
          <tr><td>${esc(periodFrom)} إلى ${esc(periodTo)}</td></tr>
        </table>
      </div>
      ...
      <table class="statement">
        ...
        <tbody>
          ${rowsHtml || `<tr><td colspan="4" class="center">لا يوجد بيانات</td></tr>`}

          <tr class="totalsRow">
            <td colspan="3" class="totalsLabel">*** إجمالي قيمة الفواتير ***</td>
            <td class="right" dir="ltr">${totalValue}</td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  if (!w) throw new Error("Popup blocked. Please allow popups for printing.");
